...
from read_data import read_budget


def get_remaining_budget(df_emissions):

    budget_start_year, budget_start_value_kt = read_budget()

    df_measured_since_budgetstart = df_emissions[df_emissions.index >=budget_start_year]
    ...
    last_measured_year_since_before_budgetstart = max(last_measured_year, budget_start_year - 1)
